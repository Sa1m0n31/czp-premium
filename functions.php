...
if ( ! function_exists( 'czp_setup' ) ) :
	...
	function czp_setup() {
		 ...
		 * If you're building a theme based on czp, use a find and replace
		 * to change 'czp' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'czp', get_template_directory() . '/languages' );
...
add_action( 'after_setup_theme', 'czp_setup' );
 ...
 */
function czp_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'czp_content_width', 640 );
}
add_action( 'after_setup_theme', 'czp_content_width', 0 );
 ...
 */
function czp_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'czp' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'czp' ),
...
add_action( 'widgets_init', 'czp_widgets_init' );
